"""
Quick Start: Click Terminal > New Terminal | pwd (check location) | cd part2/part2Exercises/tasks (if needed)
Run: python 16_input_validation.py | Check: python grade_part2.py

Please write a program which asks the user for integer numbers.

If the number is below zero, the program should print out the message "Invalid number".

If the number is above zero, the program should print out the square root of the number using the Python sqrt function.

In either case, the program should then ask for another number.

If the user inputs the number zero, the program should stop asking for numbers and exit the loop.

Remember to import sqrt: from math import sqrt

Sample Output:

Please type in a number: 16
4.0
Please type in a number: 4
2.0
Please type in a number: -3
Invalid number
Please type in a number: 1
1.0
Please type in a number: 0
Exiting...

"""

# TODO: Write your solution below this line



# Save your file and run it using: python 16_input_validation.py
# Check: python grade_part2.py

# Commit and push changes to GitHub:
# 1. git add .                                    (stage all changes)
# 2. git commit -m "Completed task 16"            (commit with message)
# 3. git push                                      (push to GitHub)
